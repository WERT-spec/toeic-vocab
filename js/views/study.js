    ...
    $('vocab-list-container').innerHTML = list.map((item, idx) => `
        <div class="vocab-list-item bg-white dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm flex items-center gap-4 active:scale-[0.98] transition-all">
            ...
            <div class="flex-[1.5] min-w-0">
                <div class="font-bold text-slate-800 dark:text-blue-400 text-lg leading-tight mb-2 whitespace-nowrap truncate">${item.w}</div>
                <div class="flex items-center gap-2 whitespace-nowrap">
                    <span class="px-1.5 py-0.5 bg-blue-50 dark:bg-blue-900/40 text-blue-500 dark:text-blue-300 italic font-bold text-[10px] rounded-md shrink-0">${item.p.replace(/[()]/g, '')}</span>
            ...
            <button onclick="playAudio('${item.w.replace(/'/g, "\\'")}', this, event)" class="w-10 h-10 flex items-center justify-center bg-blue-50 dark:bg-blue-900/40 text-blue-500 dark:text-blue-300 rounded-full shrink-0 active:scale-90 transition-all border border-blue-100/50 dark:border-blue-800/50">
